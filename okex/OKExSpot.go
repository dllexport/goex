...
func (ok *OKExSpot) GetAccountOf(curr goex.CurrencyPair) (*Account, error) {
	urlPath := "/api/spot/v3/accounts/" + curr.CurrencyA.String()
	var response struct {
		Frozen    float64 `json:"frozen,string"`
		Hold      float64 `json:"hold,string"`
		Currency  string
...
func (ok *OKExSpot) adaptOrder(response OrderResponse) *Order {
	ordInfo := &Order{
		...
		DealAmount: ToFloat64(response.FilledSize), //成交金额
